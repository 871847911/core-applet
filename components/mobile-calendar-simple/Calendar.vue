        ...
        >
          ...
          <ul class="each-month">
            ...
            >
              ...
              >
                <!-- <p class="day-tip" :style="{ color: themeColor }"
                  ><i v-text="setTip(day, item.month, item.year, 1)"></i
                ></p> -->
                <p class="day-tip" :style="{ color: themeColor }"
                  ><i v-text="setTip(day, item.month, item.year, 2)"></i
                ></p>
                <p class="day">{{ day ? day : '' }}</p>
                <p class="recent"><i v-text="setTip(day, item.month, item.year, 1)"></i></p>
              </div>
            </li>
          </ul>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      setTip(day, month, year, flag) {
        ...
        if (flag == 1) {
          const showPirceObj =
            this.datePriceList.find((item) => {
              return this.resetTime(item.date) === _date
            }) || {}
          return showPirceObj.price
        } else {
          if (this.mode == 2) {
